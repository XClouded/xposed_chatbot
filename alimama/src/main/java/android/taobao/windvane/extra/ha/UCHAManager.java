...
public class UCHAManager implements ValueCallback<Pair<Message, Message>> {
    ...
    public void onReceiveValue(Pair<Message, Message> pair) {
        ...
        if (!TextUtils.equals("pvuv", (String) map.get(EventConstants.FULLLink.LOG_TYPE))) {
            ...
            if (!TextUtils.equals("bkpg", str2)) {
                TaoLog.i(TAG, "linkID:[" + str3 + "];logData:[" + str + Operators.ARRAY_END_STR);
                return;
            }
            TaoLog.e(TAG, "linkID:[" + str3 + "];logData:[" + str + Operators.ARRAY_END_STR);
        }
    }
