...
public abstract class UTBaseConfMgr {
            ...
            while (it.hasNext()) {
                ...
                for (String str : next2.getOrangeGroupnames()) {
                    if (hashMap.containsKey(str)) {
                        ...
                        if (Logger.isDebug()) {
                            Logger.d("", "Groupname", str, "DBConfEntity", StringUtils.transStringToMap(uTDBConfigEntity.getConfContent()));
                        }
    ...
    /* access modifiers changed from: protected */
    public final synchronized void updateAndDispatch(List<UTDBConfigEntity> list, boolean z) {
        if (list == null) {
            try {
                _dispatchNonConfig();
            } catch (Throwable th) {
            ...
            List<UTDBConfigEntity> _mergeAndSaveConfig = _mergeAndSaveConfig(list);
            if (_mergeAndSaveConfig != null && _mergeAndSaveConfig.size() > 0) {
                _dispatchConfig(_mergeAndSaveConfig);
            }
        }
    }
        ...
        if (map != null) {
            ...
            if (remove != null) {
                try {
                    ...
                    Logger.e("parse Timestamp error", "timestamp", remove);
                }
            }
