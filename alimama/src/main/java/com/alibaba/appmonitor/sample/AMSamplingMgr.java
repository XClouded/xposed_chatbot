...
public class AMSamplingMgr extends UTOrangeConfBiz {
    ...
    private AMSamplingMgr() {
        ...
        for (EventType eventType : EventType.values()) {
            ...
            AMConifg buildRelation = buildRelation(Variables.getInstance().getDbMgr().find(cls, (String) null, "module,mp ASC ", -1));
    ...
    public boolean checkSampled(EventType eventType, String str, String str2) {
        return isSampled(eventType, str, str2, (Map<String, String>) null);
    }
        ...
        if (!StringUtils.isBlank(str) && map != null) {
            ...
            try {
                if (map.containsKey("event_type")) {
                    aMConifg = parseConfigEntity(cls, JSON.parseObject(map.get("event_type")));
                ...
                Variables.getInstance().getDbMgr().clear((Class<? extends Entity>) aMConifg.getClass());
                Variables.getInstance().getDbMgr().insert((List<? extends Entity>) arrayList);
            } catch (Throwable th2) {
                Logger.e("", "parse config error", th2);
            }
        }
    }
    ...
    public void enableOffline(EventType eventType, String str, String[] strArr) {
        AMConifg aMConifg;
        if (eventType != null && !TextUtils.isEmpty(str) && strArr != null && (aMConifg = this.eventTypeSamplings.get(eventType)) != null) {
