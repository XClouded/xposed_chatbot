...
public class AgooBindCache {
    ...
    public boolean isAgooRegistered(String str) {
        ...
        if (!UtilityImpl.utdidChanged(Config.PREFERENCES, this.mContext) && num != null && num.intValue() == 2) {
            return true;
        }
        return false;
    }
    ...
    private void restoreAgooClients() {
        try {
            String string = this.mContext.getSharedPreferences("AGOO_BIND", 0).getString(SP_BIND_KEY, (String) null);
            ...
            if (System.currentTimeMillis() < this.agooLastFlushTime + 86400000) {
                for (int i = 1; i < jSONArray.length(); i++) {
                    ...
                    this.mAgooBindStatus.put(jSONObject.getString("p"), Integer.valueOf(jSONObject.getInt("s")));
                }
