...
public class AliasDO extends BaseDO {
    ...
    public byte[] buildData() {
        try {
            String jSONObject = new JsonUtility.JsonObjectBuilder().put(BaseDO.JSON_CMD, this.cmd).put("appKey", this.appKey).put("deviceId", this.deviceId).put("alias", this.alias).put(JSON_PUSH_USER_TOKEN, this.pushAliasToken).build().toString();
