...
public class OrangeConfigLocal {
    ...
    public void registerListener(String[] strArr, OrangeConfigListener orangeConfigListener) {
        if (strArr != null && strArr.length != 0 && orangeConfigListener != null) {
            for (String registerListener : strArr) {
                ConfigCenter.getInstance().registerListener(registerListener, new OrangeConfigListenerStub(orangeConfigListener), true);
            }
        }
    }
    ...
    public void registerListener(String[] strArr, OrangeConfigListenerV1 orangeConfigListenerV1) {
        if (strArr != null && strArr.length != 0 && orangeConfigListenerV1 != null) {
            for (String registerListener : strArr) {
                ConfigCenter.getInstance().registerListener(registerListener, new OrangeConfigListenerStub(orangeConfigListenerV1), true);
            }
        }
    }
    ...
    public void unregisterListener(String[] strArr) {
        if (strArr != null && strArr.length != 0) {
            for (String unregisterListeners : strArr) {
                ConfigCenter.getInstance().unregisterListeners(unregisterListeners);
            }
        }
    }
