...
public class GodeyeRemoteCommandCenter {
    ...
    public void dispatchCommand(GodeyeInfo godeyeInfo) {
        ...
        for (Define.Entry next : this.mCommandControllers) {
            if (next.getOpCode().equals(godeyeInfo.commandInfo.opCode)) {
                try {
                    TraceTask traceTask = new TraceTask();
                    traceTask.toTraceTask(godeyeInfo);
                    dispatchCommandInternal((AbsCommandController) next.getValue(), traceTask, false);
                } catch (Exception e2) {
                    e2.printStackTrace();
                }
            }
