...
public class ImageStrategyExtra {
    ...
    public static ImageUrlInfo parseImageUrl(String str) {
        return parseImageUrl(str, (ImageUrlInfo) null);
    }

    static ImageUrlInfo parseImageUrl(String str, ImageUrlInfo imageUrlInfo) {
        ...
        if (sCdnRuleRegex == null) {
            sCdnRuleRegex = Pattern.compile("_(?:(?:(?:\\.webp)|(?:\\.heic))|((?:(?:(\\d+)x(\\d+)(xz)?(co0)?(c[xy]\\d+i\\d)?)|([Qq]\\d{2})|(s\\d{2,3})){1,3}(?:\\.jpg)?(?:_(?:(?:\\.webp)|(?:\\.heic)))?))(?:END_IMAGE_URL)?$");
        }
