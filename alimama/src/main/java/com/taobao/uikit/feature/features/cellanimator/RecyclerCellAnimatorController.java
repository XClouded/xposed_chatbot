...
public class RecyclerCellAnimatorController {
    ...
    public RecyclerCellAnimatorController(RecyclerView recyclerView) {
        ...
        this.mRecyclerView.setRecyclerListener(new RecyclerView.RecyclerListener() {
            public void onViewRecycled(RecyclerView.ViewHolder viewHolder) {
                Animator animator = (Animator) RecyclerCellAnimatorController.this.mAnimators.get(viewHolder.itemView);
                if (animator != null) {
                    animator.end();
                }
    ...
    private int calculateAnimationDelay(int i) {
        ...
        if (layoutManager instanceof GridLayoutManager) {
            ...
            i2 = layoutManager instanceof StaggeredGridLayoutManager ? ((StaggeredGridLayoutManager) layoutManager).getSpanCount() : 1;
        }
    ...
    private int getFirstVisiblePosition() {
        ...
        for (int i2 : ((StaggeredGridLayoutManager) layoutManager).findFirstVisibleItemPositions((int[]) null)) {
            if (i2 < i) {
                i = i2;
            }
        }
