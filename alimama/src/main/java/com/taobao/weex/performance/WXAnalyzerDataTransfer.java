...
public class WXAnalyzerDataTransfer {
            ...
            if (wXAnalyzerList != null && wXAnalyzerList.size() != 0 && (wXSDKInstance = WXSDKManager.getInstance().getAllInstanceMap().get(str)) != null) {
                try {
                    ...
                    for (IWXAnalyzer transfer : wXAnalyzerList) {
                        transfer.transfer("wxapm", wXSDKInstance.getInstanceId(), str2, jSONObject);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public static void transferInteractionInfo(WXComponent wXComponent) {
        ...
        if (isOpenPerformance && (wXAnalyzerList = WXSDKManager.getInstance().getWXAnalyzerList()) != null && wXAnalyzerList.size() != 0) {
            try {
                String jSONObject = new JSONObject().put("renderOriginDiffTime", WXUtils.getFixUnixTime() - wXComponent.getInstance().getWXPerformance().renderUnixTimeOrigin).put("type", wXComponent.getComponentType()).put("ref", wXComponent.getRef()).put(RichTextNode.STYLE, wXComponent.getStyles()).put(TemplateDom.KEY_ATTRS, wXComponent.getAttrs()).toString();
                for (IWXAnalyzer transfer : wXAnalyzerList) {
                    transfer.transfer("wxapm", wXComponent.getInstanceId(), "wxinteraction", jSONObject);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public static void transferError(WXJSExceptionInfo wXJSExceptionInfo, String str) {
        ...
        if (WXEnvironment.isApkDebugable() && (wXAnalyzerList = WXSDKManager.getInstance().getWXAnalyzerList()) != null && wXAnalyzerList.size() != 0 && (sDKInstance = WXSDKManager.getInstance().getSDKInstance(str)) != null) {
            ...
            try {
                str2 = new JSONObject().put(BindingXConstants.KEY_INSTANCE_ID, str).put("url", sDKInstance.getBundleUrl()).put("errorCode", errCode.getErrorCode()).put(ILocatable.ERROR_MSG, errCode.getErrorMsg()).put("errorGroup", errCode.getErrorGroup()).toString();
            } catch (Exception e) {
                e.printStackTrace();
                str2 = "";
            }
