...
public class WhiteScreenUtils {
    public static boolean doWhiteScreenCheck() {
        ...
        try {
            d = Double.valueOf(wxConfigAdapter.getConfig(WXInitConfigManager.WXAPM_CONFIG_GROUP, "new_ws_sampling", MessageService.MSG_DB_COMPLETE)).doubleValue();
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (random < d) {
    ...
    public static boolean isWhiteScreen(WXSDKInstance wXSDKInstance) {
        ...
        if ((containerView instanceof ViewGroup) && WXViewUtils.isViewVisible(containerView) && checkParentVisible(containerView.getParent()) && !isInWhiteList(wXSDKInstance)) {
    ...
    private static boolean isInWhiteList(WXSDKInstance wXSDKInstance) {
        ...
        String config = wxConfigAdapter.getConfig(WXInitConfigManager.WXAPM_CONFIG_GROUP, "ws_white_list", (String) null);
    ...
    private static boolean checkParentVisible(ViewParent viewParent) {
        boolean z = true;
