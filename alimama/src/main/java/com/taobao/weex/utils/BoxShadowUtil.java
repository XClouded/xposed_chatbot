...
public class BoxShadowUtil {
            ...
            view.post(WXThread.secure((Runnable) new Runnable() {
                public void run() {
                    if (Build.VERSION.SDK_INT >= 18) {
                        ...
                        if (arrayList.size() > 0) {
                            BoxShadowUtil.setNormalBoxShadow(view2, arrayList, f2, fArr2);
                        }
                        if (arrayList2.size() > 0) {
                            BoxShadowUtil.setInsetBoxShadow(view2, arrayList2, f2, fArr2);
                        }
                    }
                }
            }));
    ...
    public static class BoxShadowOptions {
        ...
        private BoxShadowOptions(int i) {
            this.viewport = FeatureFactory.PRIORITY_ABOVE_NORMAL;
            this.blur = 0.0f;
            this.spread = 0.0f;
            ...
            AnonymousClass1 r3 = new IParser() {
                public void parse(String str) {
                    if (!TextUtils.isEmpty(str)) {
                        ...
                        BoxShadowOptions.this.spread = WXViewUtils.getRealSubPxByWidth(floatValue, BoxShadowOptions.this.viewport);
            ...
            this.optionParamParsers.add(new IParser() {
                public void parse(String str) {
                    if (!TextUtils.isEmpty(str)) {
                        float floatValue = WXUtils.getFloat(str, Float.valueOf(0.0f)).floatValue();
                        BoxShadowOptions.this.blur = WXViewUtils.getRealSubPxByWidth(floatValue, BoxShadowOptions.this.viewport);
                    }
                }
            });
            this.optionParamParsers.add(r3);
        }
