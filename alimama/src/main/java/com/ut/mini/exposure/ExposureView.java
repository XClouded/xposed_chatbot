...
public class ExposureView {
    ...
    public boolean isSatisfyTimeRequired() {
        ...
        ExpLogger.d((String) null, "tag", this.tag, "duration", Long.valueOf(currentTimeMillis));
        if (currentTimeMillis <= ((long) ExposureConfigMgr.timeThreshold) || currentTimeMillis >= ((long) ExposureConfigMgr.maxTimeThreshold)) {
            return false;
        }
        return true;
    }
